...
pd <- position_dodge(.6)
carn<-ggplot(boots_C,aes(colour=Location,y=Mean,x=Species)) +
  geom_point(position=pd) +
  geom_errorbar(aes(ymin=Min,ymax=Max),width=0.4,position=pd) +
  theme(axis.ticks.x=element_blank(),
        axis.text.x=element_text(angle=45,hjust=1),
        panel.background=element_rect(fill="transparent", color=NA))

carn2<-carn +
  scale_colour_discrete(name=("Locations"))
carn3<-carn2 +
  scale_colour_manual(values=c("darkkhaki","darkolivegreen3","darkred","firebrick"), 
                      breaks=c("KBO","DDA","KUS","PAR"))



pd <- position_dodge(.6)
small<-ggplot(boots_S,aes(colour=Location,y=Mean,x=Species)) +
  geom_point(position=pd) +
  geom_errorbar(aes(ymin=Min,ymax=Max),width=0.4,position=pd) +
  theme(axis.ticks.x=element_blank(),
        axis.text.x=element_text(angle=45,hjust=1),
...
large<-ggplot(boots_L,aes(colour=Location,y=Mean,x=Species)) +
  geom_point(position=pd) +
  geom_errorbar(aes(ymin=Min,ymax=Max),width=0.4,position=pd) +
  theme(axis.ticks.x=element_blank(),
        axis.text.x=element_text(angle=45,hjust=1),
...
ggplot(boots_L,aes(Species, Mean, color=Location)) +
  ...
  geom_vline(xintercept=1:3+0.5,linetype=1,size=0.1,color="grey20")
...
LargePrey <- ggplot(boots_L,aes(Species, Mean, color=Location)) +
  geom_point(position=pd) +
  geom_errorbar(aes(ymin=Min,ymax=Max,position=pd),width=1.0,position=pd) +
  theme_bw() + 
  opts(axis.text.x=theme_text(angle=90,hjust=1))
...
LargePrey +
  scale_colour_discrete(name=("Hunting pressure")) +
  scale_colour_manual(values=c("goldenrod","firebrick","forestgreen"),
                      breaks=c("Me","Lo","VL"),
                      labels=c("Medium","Low","Very low"))

SmallPrey <- ggplot(boots_S,aes(Species, Mean, color=Hunt2)) +
  geom_point(position=pd) +
  geom_errorbar(aes(ymin=Min,ymax=Max,position=pd),width=1.0,position=pd) +
  theme_bw() + 
  opts(axis.text.x=theme_text(angle=90,hjust=1))
...
Predator <- ggplot(boots_C,aes(Species, Mean, color=Hunt2)) +
  geom_point(position=pd) +
  geom_errorbar(aes(ymin=Min,ymax=Max,position=pd),width=1.0,position=pd) +
  theme_bw() + 
  opts(axis.text.x=theme_text(angle=90,hjust=1))
